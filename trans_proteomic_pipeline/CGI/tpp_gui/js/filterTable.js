 ...
 *
 * Minor modifications made for TPP/Petunia.  LM, 2011
 */
...
/*
 * Transform back each
 * <span>preText <span class="remfileSelected">term</span> postText</span>
 * into its original
...
function dehighlight(container) {
	for (var i = 0; i < container.childNodes.length; i++) {
		...
		if (node.attributes && node.attributes['class']
			&& node.attributes['class'].value == 'remfileSelected') {
...
function create_node(child) {
	...
	node.setAttribute('class', 'remfileSelected');
	node.attributes['class'].value = 'remfileSelected';
...
for (var t = 0; t < tables.length; t++) {
	...
	if (element.attributes['class']
		...
		if (document.getElementById("filter_here")) {
			document.getElementById("filter_here").appendChild(form);
		} else {
