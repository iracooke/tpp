	  ...
	  if Ext.lower() == ".mzxml":
        ...
        for Line in InspectHandle.xreadlines():
            ...
            try:
              ...
              print "WARNING: malformed FileOffset %s in/after scan %d" % (Bits[self.Columns.FileOffset], LastScanNumber)
              continue

            try:
              ScanNumber = int(Bits[self.Columns.ScanNumber])
            except:
              print "WARNING: malformed ScanNumber %s in/after scan %d" % (Bits[self.Columns.ScanNumber], LastScanNumber)
              continue
