...
unsigned long mscore_k::mconvert(double _m, const double _z) // added in cyclone merge
{
/*
...
// TODO merge from cyclone, what should this look like
cerr << "mscore_k::mconvert(double _m, const double _z) not implemented yet";
exit(-1);  
// i'm guessing just: return imass((_z*m_pSeqUtilFrag->m_dProton + _m)/_z)
// but it would be well to understand the cyclone changes in mscore_tandem
return 0;
}
...
float mscore_k::ion_check(const unsigned long _v,const size_t _s)
{
	// TODO new in cyclone mscore_tandem, so we need similar?
	cerr << "mscore_k::ion_check not yet implemented";
	exit(-1);
}
 ...
 * add_seq.
 */
float mscore_k::score(const size_t _i)
{
	m_fScore = -1.0;
	m_fHyper = -1.0;
	double dFactor = 1.0;
