...
IF EXIST boost_1_45_0\stage\lib\libboost_zlib-vc100-mt-sgd.lib GOTO DONE
:PREPARE
call VCVARS32.BAT
...
..\bjam --toolset=msvc-10.0 "-sNO_ZLIB=0" -sZLIB_SOURCE="%1/zlib-1.2.5" release debug link=static threading=multi runtime-link=static --with-regex --with-filesystem --with-iostreams --with-thread --with-program_options --with-serialization --with-system --with-date_time stage define=_HAS_ITERATOR_DEBUGGING=0
