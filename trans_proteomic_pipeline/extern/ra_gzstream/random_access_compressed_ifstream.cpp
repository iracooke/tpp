...
//Breakout version: 2011-07-15 mhoopmann
//Compiles as standalone object without pwiz or boost.
//Useful for test purposes, and lightweight compilations.
//Also useful utility for parsing any gzipped file, not just mass spec data files.
...
#define CHUNK 32768//16384         /* file input buffer size */
/* access point entry */
class synchpoint {
public:
...
private:
   ...
   synchpoint *addIndexEntry(random_access_compressed_ifstream_off_t in, random_access_compressed_ifstream_off_t out);
	...
	std::streampos get_next_read_pos() {
		...
		return outbuf_headpos+(std::streampos)(gptr()-(char *)outbuf);
	}
...
//			boost::iostreams::position_to_offset(this->last_seek_pos); // last requested absolute uncompress filepos
			(random_access_compressed_ifstream_off_t)this->last_seek_pos;
      ...
      assert(strm.total_in == synch->in-this->start);
		...
		this->infile->seekg((std::streampos)synch->in);
...
std::streampos random_access_compressed_streambuf::my_seekg(std::streampos offset, std::ios_base::seekdir whence,std::ios_base::openmode Mode) {
	...
	if (0==offset) {
		...
		if ((whence == std::ios_base::beg) && // just a rewind
				...
				(offset==outbuf_headpos)) // or buffer is not yet loaded, let underflow() do it
			{
				...
				this->infile->seekg((std::streampos)this->start);
	...
	if (whence == std::ios_base::cur) {
		...
		pos += (std::streampos)this->uncompressedLength;
	}
