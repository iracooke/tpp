...
ASAPRatioPeptideParser::ASAPRatioPeptideParser(const char* xmlfile, const InputStruct &options, const char *testMode, double mzBound) : Parser("asapratio") {
  ...
  spectrum_->size = 10;
  spectrum_->xval = new double[_MZXML_READER_MXSCANNUM_];
  spectrum_->yval = new double[_MZXML_READER_MXSCANNUM_];
  pInput_ = options;
  verbose_ = false;
...
/*
  This function converts "char *string" to "residueStrct AA". 
  aa->sz = -1 if invalid input.
*/
...
void ASAPRatioPeptideParser::evalPepDataStrct(char *pepSeq, long scan, int chrg, char *xmlFile,
  ...
  //char text[10];
  /*
  cout << pInput_.szPeptide << ": ";
  
    if(modinfo_ == NULL)
      cout << "null" << endl;
    else
      cout << modinfo_->getNumModAAs() << endl;
  */
  ...
  if(data_.indx < 0){
    ...
    //cout << "normal return for " << pepSeq << ": index: " << data_.indx << endl;
    return;
  }
  */
}
...
#ifdef USE_STD_MODS
/*
// this will make peptide mass unnecessary for calculation
double ASAPRatioPeptideParser::getModifiedPeptideMass(char* peptide, ModificationInfo* modinfo, Boolean monoisotopic) {
  double mass = 0.0;
...
Boolean ASAPRatioPeptideParser::setModifiedPeptideMasses(char* peptide, ModificationInfo* modinfo, Boolean monoisotopic, double* light, double* heavy) {
...
//////////////////////////////////////////////////////////////////////////
/*
  For a given sequence "char * sequence" of charge "int qState", 
  this function gives its (mono-isotopic mass)/charge.
*/
		    ...
		    residueStrct *modifiedAA, int modAANum)
{
  ...
  //cout << "num: " << modAANum << " chr: " << qState << endl;
  //for(int k = 0; k < modAANum; k++)
  // cout << modAAs_[k].sz << " " << modAAs_[k].rp << " " << modAAs_[k].ms << endl;
		     ...
		     char *pngFileBase, int cgiIndx)
{
  ...
  //  cout << "here in pepdatastrct" << endl;
  ...
  */
  ...
  //  cout << data->chrg-1 << " vs " << _ASAPRATIO_MXQ_ << " and " << cidIndx << " vs 2" << endl;

  /*
  for (i = 0; i < _ASAPRATIO_MXQ_; ++i){
    cout << "I: " << i << " of " << _ASAPRATIO_MXQ_ << endl;
    for (j = 0; j < 2; ++j) {
      cout << "j: " << j << endl;
      cout << "index: " << data->peaks[i][j].indx << endl;
      if(data->peaks[i][j].indx >= 0) {
	for (k = 0; k < lcSpect.rawSpectrum[i][j].size; ++k)
	  printf("spect: %lf %lf\n", lcSpect.rawSpectrum[i][j].xval[k], lcSpect.rawSpectrum[i][j].yval[k]);
	//freeSpectStrct(lcSpect.rawSpectrum[i][j]);	
	//freeSpectStrct(lcSpect.fitSpectrum[i][j]);	  
      }  
    }
  }
  */
  ...
  */
  ...
  //  cout << "ready to free" << endl;
  // reset data->indx;
  if(data->indx == 0)
    data->indx = 1;

  /*
  Array<Tag*>* ts = generateXML(1, True, True);
		   ...
		   double timeWd, char *xmlFile,int smoothItrs,double smoothRTwindow)
{
  ...
  */
  ...
  lcSpect->rawSpectrum[i][j] = *tmpSpect;
      //cout << "done1 with j: " << j << " and i: " << i << endl;
  lcSpect->fitSpectrum[i][j] = *tmpSpect;
      //cout << "done2 with j: " << j << " and i: " << i << endl;
  smoothSpectFlx(&(lcSpect->fitSpectrum[i][j]), fitTime, 10);
      
      //      cout << "done with copy for " << i << " and " << j << endl;
    }//for (j = 0; j < 2; ++j) {
  } //  for (i = 0; i < _ASAPRATIO_MXQ_; ++i){
  ...
  */
  ...
  //  free(lcSpect->expScanNums);
  //  cout << "done freeing lcspect" << endl; e


  //lcSpect->expScanNums = new long[size];
  lcScan = 0;
 ...
 //free(xmlIndx_); // added by AK....taken away by AK 7.15.04
  ...
  //delete lcSpect->expScanNums;
  //free(lcSpect->expScanNums);
  //cout << "done freeing exp" << endl;
  //  exit(1);
  return 1;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////
/*
  This function gets LC spectrum by summing up ion intensities in MS spectra within a series of 
  mass windows.
*/
			   ...
			   xmlIndxStrct *xmlIndx)
{
  ...
  //printf("OK1.1.5.1.5 %d %d %lf %d \n", iSt, iNd, dMz, xmlIndx->totScan);
  if(iSt > iNd)
    return NULL;
...
/////////////////////////////////////////////////////////////////////////////
/*
  For a given time, this function returns the closest scan number with
  fabs(scanTime-time) minimium.
*/
int ASAPRatioPeptideParser::getScanNum(double time, xmlIndxStrct *xmlIndx)
{
  ...
  //cout << "scan0: " << scanTime[0] << " bnd: " << bnd[0] << " vs time: " << time << endl;
  if(scanTime[0] >= time)
    return bnd[0];
  ...
  //cout << "scan1: " << scanTime[1] << " bnd: " << bnd[1] << " vs time: " << time << endl;
  ...
  while(bnd[1]-bnd[0] > 1) {
    ...
    //cout << "scantime0: " << scanTime[0] << " bnd: " << bnd[0] << ", scantime1: " << scanTime[1] << " bnd: " << bnd[1] << endl;
...
{
  /*
  int spectPeak(spectStrct spectrum, int position, int direction);
  int findNextValley(spectStrct spectrum, int position, int direction);
  */
  int i;
