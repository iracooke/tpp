...
//#define PROG_NAME "asapratio_pvalue"
...
class ASAPRatioPvalueParser : public Parser {
 ...
 protected:
  ...
  char* getBofFile(char* interactfile);
  ...
  void ASAPRatio_freeProDataStrct(proDataStrct* data);
  double normalize(double* ratio);
  void setModel(double* ratios); // or setModel(double** ratios);

  Boolean heavy2light_;
  proDataStrct* pro_ratio_; // the ASAP protein object
  Boolean use_seq_ratios_; // whether to use Sequence object ratios rather than protein ratios for
                           // normalization model
  char* pngfile_;
