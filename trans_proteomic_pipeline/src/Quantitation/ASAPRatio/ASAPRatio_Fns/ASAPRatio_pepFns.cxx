...
int getLCSpectAreaAndTime(double *area, double *peakTime, 
			  ...
			  const spectStrct &fitSpectrum, 
			  int pkScan, int *valleyScan, double background) {
  return getLCSpectAreaAndTime(area, peakTime, 
			       rawSpectrum, fitSpectrum, 
			       pkScan, valleyScan,  background, 0, False);
		 ...
		 const lcSpectStrct &lcSpect, int qState)
{
  ...
  for (j = 0; j < 2; ++j) { 
	...
	fprintf(file, "set arrow from  %ld, graph 0 to %ld, graph 0.9 head size screen 0.01,90 lt 8 lw 4\n",
		cidTime, cidTime);
	fprintf(file, "set label \"CID\" at %ld, graph 0.85 tc lt 8\n",
		cidTime);
      }
      else {
	...
	fprintf(file, "'%s't \"light +%d, m/z %.4f\"w l lt 1 lw 2, ",
		datFiles[0][j], qState, tmp_mz);
      }
      else { 
	tmp_mz= (data.msHeavy+(qState-1)*_ASAPRATIO_HM_)/(double)qState;
	fprintf(file, "'%s't \"heavy +%d, m/z %.4f\"w l lt 1 lw 2, ",
		datFiles[0][j], qState, tmp_mz);
      }
      // fitSpectrum
 ...
 // gnuCommand = (char *) calloc(strlen(gnuFile)+strlen(GNUPLOT_BINARY)+10, sizeof(char));
