...
off_t getIndexOffset(FILE * pFI)
{
   ...
   for (indexOffsetOffset = -120; ; indexOffsetOffset++ )
   {
      ...
      fseeko(pFI, indexOffsetOffset, SEEK_END);
      ...
      off_t lScanIndex)
{
   ...
   if ((pPeaks = (float *) malloc((peaksCount+1) * sizeof(float) * 2)) == NULL ||
       (pDecoded = (char *) malloc((peaksCount+1) * sizeof(float) * 2)) == NULL)
   {
      printf("Cannot allocate memory\n");
      exit(1);
   }
