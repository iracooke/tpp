...
#include "mzXML/common/ScanConverter.h"
#include <iostream>
							 ...
							 mzIntB64StringEncodedByteLen_(-1)
{
	...
	/*	for (long c=0; c<mzRatioArrayLength_; c++){
		...
		long mzIntIndex = 0;
		long j = 0;
		for (long c=0; c<mzIntArrayLength_/2; c++){
		//for (long c=0; c<mzRatioArrayLength_; c++){
			//cout << (float) scan->mzArray_[j] << ";" << (float) scan->intensityArray_[j] << endl;
			mzIntArray[mzIntIndex++] =(float) scan->mzArray_[j];
			mzIntArray[mzIntIndex++] = (float) scan->intensityArray_[j];
			j++;
		...
		//long max = 10;
		//if (max > scan->getNumDataPoints()) {
		//	max = scan->getNumDataPoints();
		//}
		//mzIntIndex = 0;
		//for (int i=0; i<max; i++) {
		//	cout << "  (" << mzIntArray[mzIntIndex++] << "," <<
		//		mzIntArray[mzIntIndex++] << ")" << endl;
		//}
