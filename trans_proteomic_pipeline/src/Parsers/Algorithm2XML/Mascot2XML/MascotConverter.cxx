...
Date          : 11.27.02 
SVN Version   : $Id: MascotConverter.cxx 6590 2014-08-22 20:21:52Z slagelwa $
...
void MascotConverter::init(const char* filename, Boolean xml, Boolean generateTgz, Boolean generateDta) {
  ...
  pwiz::util::random_access_compressed_ifstream fin(filename /* can read gzipped results */
  ...
  while(fin.getline(line, line_width)) {
    ...
    // peptides
      ...
      if(queryIndex == spectra_.length() + 1) {
        if(line_len > 8 && line[0] == 'n' && line[1] == 'u' && line[2] == 'm' && line[3] == '_' &&
	  ...
	  if (!strncmp(nextspectrum, "ScanNumber:",11)) {
      ...
      else if(queryIndex == spectra_.length() && line_len > 6 && line[0] == 'I' && line[1] == 'o' && line[2] == 'n' && line[3] == 's' && line[4] == '1' && line[5] == '=') {
...
#ifdef WINDOWS_CYGWIN  // make sure it's nice and cygwinish
  char szCommand[SIZE_FILE+50];
  char szBuf[SIZE_FILE];
  FILE *fp;
  // must first pass to cygpath program
  sprintf(szCommand, "cygpath -u '%s'", file);
  if((fp = popen(szCommand, "r")) == NULL) {
    printf("cygpath error, exiting\n");
    exit(1);
  }
  else {
    fgets(szBuf, SIZE_FILE, fp);
    pclose(fp);
    szBuf[strlen(szBuf)-1] = 0; // kill newline     
  }
  file = szBuf;   
#endif
...
#define WRITEDASHES(n) {if ((n)>0) {dashes[n]=0;output.append(dashes);dashes[n]='-';}}
  ...
  snprintf(buf,sizeof(buf),"<A TARGET=\"Win1\" HREF=\"%splot-msms.cgi?Dta=%s%s/%s.dta&amp;MassType=%d&amp;NumAxis=1&amp;ISeries=010000011&amp;Pep=%s", CGI_BIN, fullpath_, filextn_, spectra_[index], mass_type_, peptides_[index]);
  ...
  snprintf(buf,sizeof(buf), "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Ref=%s&amp;Db=%s&amp;Pep=%s&amp;MassType=%d\">%s", CGI_BIN, proteins_[index], database_, peptides_[index], mass_type_, proteins_[index]);
  output.append(buf);
  output.append("</A>");
  ...
  if(pluses_[index]) {
    ...
    snprintf(buf,sizeof(buf), "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Db=%s&amp;Pep=%s&amp;MassType=%d&amp;sample_enzyme=%s&amp;min_ntt=%d\">+%d</A>", CGI_BIN, database_, peptides_[index], mass_type_, sampleEnzymeInstance_->getName(), min_ntt, pluses_[index]);
    output.append(buf);
  }
  else
    output.append("    ");
  ...
  for(k = alt_inds_[index]; k < stop; k++) {
    ...
    snprintf(buf,sizeof(buf),"<A TARGET=\"Win1\" HREF=\"%splot-msms.cgi?Dta=%s%s/%s.dta&amp;MassType=%d&amp;NumAxis=1&amp;ISeries=010000011&amp;Pep=%s", CGI_BIN, fullpath_, filextn_, spectra_[index], mass_type_, alt_peptides_[k]);
    output.append(buf);
    ...
    snprintf(buf,sizeof(buf), "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Ref=%s&amp;Db=%s&amp;Pep=%s&amp;MassType=%d\">%s", CGI_BIN, alt_proteins_[k], database_, alt_peptides_[k], mass_type_, alt_proteins_[k]);
    output.append(buf);
    output.append("</A>");
	...
	output.append(" ");
      ...
      int min_ntt = getMinNumberTermini();
      snprintf(buf,sizeof(buf), "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Db=%s&amp;Pep=%s&amp;MassType=%d&amp;sample_enzyme=%s&amp;min_ntt=%d\">+%d</A>", CGI_BIN, database_, alt_peptides_[k], mass_type_, sampleEnzymeInstance_->getName(), min_ntt, alt_pluses_[k]);
      output.append(buf);
    }
    else
      output.append("    ");
...
void MascotConverter::databaseSearch() {
  ...
  pwiz::util::random_access_compressed_ifstream fdata(database_); // can read gzipped results
  ...
  for(k = 0; k < spectra_.length(); k++) {
    ...
    if(checkResult(k)) {
      ...
      if(counter < 10)
        fprintf(fout, " ");
      counter++;
      fprintf(fout, "<A TARGET=\"Win1\" HREF=\"%s%s?OutFile=", CGI_BIN, outfilecgi);
      ...
      fprintf(fout, "<A TARGET=\"Win1\" HREF=\"%ssequest-tgz-out.cgi?Dta=%s%s%c%s.dta&amp;MassType=%d&amp;NumAxis=1&amp;ISeries=010000011&amp;Pep=%s\">", CGI_BIN, fullpath_, filextn_, slash, spectra_[k], mass_type_, peptides_[k]);
      ...
      fprintf(fout, "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Ref=%s&amp;Db=%s&amp;Pep=%s&amp;MassType=%d\">%s", CGI_BIN, proteins_[k], database_, peptides_[k], mass_type_, proteins_[k]);
      fprintf(fout, "</A>");
      ...
      if(pluses_[k]) {
        ...
        fprintf(fout, "<A TARGET=\"Win1\" HREF=\"%scomet-fastadb.cgi?Db=%s&amp;Pep=%s&amp;MassType=%d\">+%d</A>", CGI_BIN, database_, peptides_[k], mass_type_, pluses_[k]);
      }
      else
        fprintf(fout, "    ");
      fprintf(fout, "  ");
...
Boolean MascotConverter::validate_db() {
  ...
  if(!(database_ == NULL || strcmp(database_, "") == 0)) {
    ...
    pwiz::util::random_access_compressed_ifstream ftest(database_); // can read gzipped files
    if( ftest.is_open()) {
      result = True;
      ftest.close();
    }
