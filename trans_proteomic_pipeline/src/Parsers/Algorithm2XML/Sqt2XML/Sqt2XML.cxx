...
int main(int argc, char** argv) {
	...
	if( argc < minArgs + flagArgs + 1 ) {
		...
		cerr << "Usage: " << argv[0] << " [OPTIONS] <filepath to SQT file>" << endl << endl;
		cerr << "   OPTIONS:" << endl;
		cerr << "      -r <max. result rank to keep, default 10>" << endl;
		cerr << "      -p <filepath to sequest.params>" << endl;
		cerr << "      -E <enzyme> " << endl
		...
		<< "         chymotrypsin - Cut: YWFM, No Cut: P, Sense: C-term " << endl
		...
		<< "         lysn_promisc - Cut: KASR, No Cut: none, Sense: N-term " << endl
   ...
   //outputFilename = inputFilename.substr( 0, inputFilename.find_last_of('.')+1 ) + "pep.xml";
	...
	mzXmlPath_ = rampConstructInputFileName(inputFilename.substr( 0, inputFilename.find_last_of('.')+1 ));
