	  ...
	  if (isAttr("id", name))
	    {
	      ...
	      scanCur.scanStart = index;
	      scanCur.scanEnd = index;
	    }
...
bool TandemResultsParser::writePepXML()
{
  ...
  initAminoAcidMasses(dMassAA, type);
  ...
  if (pf != NULL)
    ...
    {
      // TODO: Add command-line flag to allow conversion without
      //       mzXML file.
      cerr << "WARNING: Failed to open mzXML file.\n"
	   << "         Output will not contain retention times.\n";
      //		cerr << "ERROR: Failed to open mzXML file.\n";
      //		return false;
    }
  if (rawDataXMLext == NULL)
    ...
    {
      strBaseName = strMzXMLFile.substr(0,
					strMzXMLFile.length() - strlen(rawDataXMLext));
    }
...
bool TandemResultsParser::writePepXML(ostream& out, const char *baseName, const char *rawDataXMLext, const char *fragmentType)
{
  ...
  readMods(paramsHandler.getUsedParam("residue, modification mass"), false);
  ...
  readMods(paramsHandler.getUsedParam("residue, potential modification mass"), true);
  ...
  for (;;)
    {
      ...
      if (!readMods(paramsHandler.getUsedParam(residueParam), true))
	break;
    }

  // Refine variable modifications
  readMods(paramsHandler.getUsedParam("refine, potential modification mass"), true);
  ...
  for (;;)
    {
      ...
      if (!readMods(paramsHandler.getUsedParam(refineParam), true))
	break;
    }
  readMods(paramsHandler.getUsedParam("refine, potential N-terminus modifications"), true);
  readMods(paramsHandler.getUsedParam("refine, potential C-terminus modifications"), true);
...
bool TandemResultsParser::readMods(string mod, bool variable)
{
  if (mod.length() < 3)
    return false;
...
//		}
	...
	termCDiff += dCTermProt;
	  ...
	  while (it != vMods.end())
	      ...
	      // Allow only 1 static and 1 variable terminal modification.
