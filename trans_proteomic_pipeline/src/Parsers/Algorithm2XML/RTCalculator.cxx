...
*  FILENAME      :   pi.c
*
*  DESCRIPTION   :   Given a protein sequence, these functions return its RT.
* 
...
double RTCalculator::getSSRCalcHP(char* peptide) {
  ...
  sprintf(szBuf, "SSRCalc=%s", LOCAL_BIN);
  putenv(szBuf);

  sprintf(szBuf, "%sSSRCalc3.pl --alg 3.0 --seq %s --output tsv | cut -f3", LOCAL_BIN, peptide);
...
void RTCalculator::batchRTCalc() {
  ...
  for (int dd = 0; dd < (int)rts_.size(); dd++) {
    ...
    double predictedRT = calc_PepRT(pep);
...
void RTCalculator::batchRTCalc(const char* catfile) {
  ...
  if (rt_catalog_ == NULL) {
	...
	if (nextRT <= 0) nextRT = calc_PepRT(pep);
...
void RTCalculator::batchSSRCalcHP() {
  ...
  sprintf(szBuf, "%sSSRCalc3.pl --alg 3.0 --source %s --output tsv | cut -f3", LOCAL_BIN, pep_file_str);
  FILE* pipe;
  if ((pipe = tpplib_popen(szBuf, "r")) == NULL) {
    cout << "error calling SSRCalc3.pl" << endl;
    exit(1);
...
void RTCalculator::write_RTann(string& file) {
  if (file.empty()) 
    file = DEFAULT_LOCAL_BIN + (string)"RTtrain";
...
void RTCalculator::read_RTann() {
  string file =  DEFAULT_LOCAL_BIN + (string)"RTtrain.fann";
  ann_file_ = file;
...
Boolean RTCalculator::recalc_RTstats(Array<double>* probs, double min_prob, Array<int>* ntts, int min_ntt) {
  return recalc_RTstats( probs, min_prob, ntts,  min_ntt, 2700);
}
...
double RTCalculator::calc_GradientCorrection(double slope , double intercept) {
  ...
  if ( run_name_->find("NG_DP_Reto_2ndy_2-NG_reto_1663_13a_1_448") != string::npos) {
    cerr << "DDS: DEBUG" <<endl;
  }
...
//     double dis_mean=0, dis_sd=0;
//     int c = 0;
//     for (i=0; i<(int)rts_.size(); i++) {
//       if (rts_[i] > 0 && (*used_)[i]) {
// 	gsl_vector_set(dis, c++, fabs(( run_slope_ * rts_[i] +  run_intercept_ )- (*RTs_)[i]));    
//       }
//     }
    
//     dis_mean = gsl_stats_mean(gsl_vector_ptr(dis, 0), 1, run_RT_used_count_);
//     dis_sd = gsl_stats_sd(gsl_vector_ptr(dis, 0), 1, run_RT_used_count_);
//     gsl_vector_free(dis);
    
    //  Array<bool>* use = new Array<bool>(rts_.size());
    //used_count_ = orig_used_count;
	...
	if (fabs(( slope * rts_[i] +  intercept) -(*RTs_)[i]) > max_dis) {
