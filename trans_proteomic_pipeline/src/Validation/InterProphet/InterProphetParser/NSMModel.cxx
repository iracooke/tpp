...
void NSMModel::insert(double prob, double val) {
  if (prob < 0 || prob > 1) {
    return;
  }
...
double NSMModel::getPosProb(double val) {
  //TODO: smoothing
  int i = 0;
  while (i < num_bins_ - 1  && val > (*binthresh_)[i] ) {
    i++;
...
double NSMModel::getNegProb(double val) {
  //TODO: smoothing
  int i = 0;
  while (i < num_bins_ - 1 && val > (*binthresh_)[i] ) {
    i++;
