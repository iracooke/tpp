...
static map<string, MSGFDBDiscrimFunctionFactory*>* pFactoryMap_ = NULL;

void MSGFDBDiscrimValMixtureDistr::registerDiscrimFunctionFactory(MSGFDBDiscrimFunctionFactory* factory) {
  if (pFactoryMap_ == NULL)
    pFactoryMap_ = new map<string, MSGFDBDiscrimFunctionFactory*>();
  string name = factory->getName();
  pFactoryMap_->insert(make_pair(name, factory));
}

// TODO: Figure out a better way to make sure these get linked in.
//extern void linkMSGFDBPscore();
extern void linkMSGFDB_MSGFScore();

MSGFDBDiscrimValMixtureDistr::MSGFDBDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof) {
  init(charge, engine, name, tag, maldi, qtof, False, True);													   
}																												   
																												   
MSGFDBDiscrimValMixtureDistr::MSGFDBDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
} 

MSGFDBDiscrimValMixtureDistr::MSGFDBDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, nonparam);
} 
void MSGFDBDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) {
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
}
void MSGFDBDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) {
  if (bFirstCall) {
	bFirstCall = false;
	// put the text of the "README" file where users can actually be aware of it - BSP
	std::cout << "WARNING!! The discriminant function for MSGFDB is not yet complete.  It is presented "
			"here to help facilitate trial and discussion.  Reliance on this "
  ...
  if (!nonparam) {
    cerr << "WARNING: MSGFDB only support semi-parametric PeptideProphet modelling, which relies on a DECOY search." << endl;
  }
  nonparam_ = True; //TODO: clean up code from parameteric stuff that doesn't apply to MSGFDB
  //  nonparam_ = nonparam;

  //  linkMSGFDBPscore();
  linkMSGFDB_MSGFScore();
  ...
  map<string, MSGFDBDiscrimFunctionFactory*>::iterator it = pFactoryMap_->find(algorithmName);

  if (it == pFactoryMap_->end()) {
    cerr << "Error: MSGFDB scoring algorithm '" << algorithmName << "' is not supported.";
    exit(1);
  }
  ...
  if (nonparam_) {
    negdistr_ = new NonParametricDistribution(0.05, True, False);
    posdistr_ = new NonParametricDistribution(0.05, False, False);
  }
  else {
    if(gammapos_) 
...
Boolean MSGFDBDiscrimValMixtureDistr::update(Array<double>* probs) {
...
void MSGFDBDiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
    ((MSGFDBDiscrimFunction*)discrim_func_)->setMassSpecType(mass_spec_type);
}



double MSGFDBDiscrimValMixtureDistr::getPosProb(int index) { 
...
double MSGFDBDiscrimValMixtureDistr::getNegProb(int index) {
  if(all_negs_ || (*doublevals_)[index] < negmean_) {
    return 1.0;
  }
...
Boolean MSGFDBDiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
double MSGFDBDiscrimValMixtureDistr::getMinVal() {
  return (DiscrimValMixtureDistr::getMinVal());
  
}

double MSGFDBDiscrimValMixtureDistr::getMaxVal() {
