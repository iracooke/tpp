#include "PhenyxDiscrimValMixtureDistr.h"
...
Program       : PhenyxDiscrimValMixtureDistr for PeptideProphet                                                       
...
static map<string, PhenyxDiscrimFunctionFactory*>* pFactoryMap_ = NULL;

void PhenyxDiscrimValMixtureDistr::registerDiscrimFunctionFactory(PhenyxDiscrimFunctionFactory* factory) {
  if (pFactoryMap_ == NULL)
    pFactoryMap_ = new map<string, PhenyxDiscrimFunctionFactory*>();
  string name = factory->getName();
  pFactoryMap_->insert(make_pair(name, factory));
}

// TODO: Figure out a better way to make sure these get linked in.
extern void linkPhenyxZscore();

PhenyxDiscrimValMixtureDistr::PhenyxDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof) {
  init(charge, engine, name, tag, maldi, qtof, False, False);												 
}																											 
																											 
PhenyxDiscrimValMixtureDistr::PhenyxDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, False);												 
}																											 
																											 
PhenyxDiscrimValMixtureDistr::PhenyxDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, nonparam);
} 
void PhenyxDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) {
  init(charge, engine, name, tag, maldi, qtof, gamma, False);
}
void PhenyxDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) {
  if (bFirstCall) {
	bFirstCall = false;
	// put the text of the "README" file where users can actually be aware of it - BSP
	std::cout << "WARNING!! The discriminant function for Phenyx is not yet complete.  It is presented "
			"here to help facilitate trial and discussion.  Reliance on this "
  ...
  map<string, PhenyxDiscrimFunctionFactory*>::iterator it = pFactoryMap_->find(algorithmName);

  if (it == pFactoryMap_->end()) {
    cerr << "Error: Phenyx scoring algorithm '" << algorithmName << "' is not supported.";
    exit(1);
  }
...
void PhenyxDiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
    ((PhenyxDiscrimFunction*)discrim_func_)->setMassSpecType(mass_spec_type);
}



double PhenyxDiscrimValMixtureDistr::getPosProb(int index) { 
...
double PhenyxDiscrimValMixtureDistr::getNegProb(int index) {
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegDistribution(NTTMixtureDistr* nttdistr) {
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegGammaDistribution(NTTMixtureDistr* nttdistr) {
  ...
  double negstdev[] = {23, 23, 23, 23, 23, 23, 23};

  // values for truncating negative distribution
  if (charge_ != 0) {
    ((GammaDistribution*)(negdistr_))->
    setDistrMinval(((PhenyxDiscrimFunction*)discrim_func_)->getMinVal());    
  }
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegExtremeValueDistribution(NTTMixtureDistr* nttdistr) {
  ...
  // values for truncating negative distribution
  if (charge_ != 0) {
    ((ExtremeValueDistribution*)(negdistr_))->setDistrMinval(((PhenyxDiscrimFunction*)discrim_func_)->getMinVal());
  }
  ...
  for(k = 0; k < getNumVals(); k++) {
    //Use only NTT=0 values to init the negative, 
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegGammaDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    return True;
  }
  // never use NTT=0 to initialize negative distribution
...
Boolean PhenyxDiscrimValMixtureDistr::initializeNegExtremeValueDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    return True;
  }
...
void PhenyxDiscrimValMixtureDistr::setNegativeDistr(double mean, double stdev) {
  assert(!gamma_);
...
double PhenyxDiscrimValMixtureDistr::getMinVal() {
  return (DiscrimValMixtureDistr::getMinVal());
  
}

double PhenyxDiscrimValMixtureDistr::getMaxVal() {
