...
*/
DiscrimValMixtureDistr::DiscrimValMixtureDistr(int charge, const char* name, const char* tag, Boolean gamma, Boolean maldi, Boolean qtof, Boolean nonparam, bool use_expect)
 : MixtureDistr(charge, name, tag), isdecoy_(0), consStdev_(0) {  
...
void DiscrimValMixtureDistr::setConservative(float stdev) {
  consStdev_ = stdev;
}

void DiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
  discrim_func_ = new AbbrevSequestDiscrimFunction(charge_, use_expect_);
}

bool DiscrimValMixtureDistr::valid(SearchResult* result) {
 
  if(discrim_func_ != NULL) {
   double nextval = discrim_func_->getDiscriminantScore(result);
   if ( isnan(nextval) || isinf(nextval) ) {
     cerr << "ERROR: Discriminant value computed as " << nextval << ", for spectrum " << result->spectrum_ 
...
void DiscrimValMixtureDistr::enter(SearchResult* result) {
  if(discrim_func_ != NULL) {
    double nextval = discrim_func_->getDiscriminantScore(result);
...
double* DiscrimValMixtureDistr::copy(double* init, int num) {
...
Boolean DiscrimValMixtureDistr::noDistr() {
  //if (nonparam_) {
  //  return False;
  //}
  // if (charge_ == 0) {
...
double DiscrimValMixtureDistr::getPosProb(int index) {
...
double DiscrimValMixtureDistr::getNegProb(int index) {
  if(all_negs_ || (*doublevals_)[index] < negmean_) {
    return 1.0;
  }
  else if (nonparam_) {
...
void DiscrimValMixtureDistr::enter(int index, double val) {
...
double DiscrimValMixtureDistr::getRightCumulativeNegProb(int index, double right_val) {
  return negSlice((*doublevals_)[index], right_val);
}

double DiscrimValMixtureDistr::getRightCumulativeNegProb(double total, int index, double right_val) {
  return ((ContinuousDistribution*)(negdistr_))->slice(total, (*doublevals_)[index], right_val);
}

int DiscrimValMixtureDistr::slice(double left_val, double right_val) {
  int tot = 0;
...
Boolean DiscrimValMixtureDistr::decayMultimixture() { return qtof_ || (charge_ == 0 && maldi_); }

double DiscrimValMixtureDistr::posSliceWithNTT(double left_val, double right_val, int ntt) {
...
double DiscrimValMixtureDistr::posSlice(double left_val, double right_val) {
  if(decayMultimixture())
...
double DiscrimValMixtureDistr::negSlice(double left_val, double right_val) {
  return ((ContinuousDistribution*)(negdistr_))->slice(left_val, right_val);
}

void DiscrimValMixtureDistr::setNegativeDistr(double mean, double stdev, double zero) {
...
void DiscrimValMixtureDistr::setPositiveDistr(double mean, double stdev) {
...
void DiscrimValMixtureDistr::setPositiveDistr(double mean, double stdev, double tot) {
  double* next = new double[2];
  next[0] = mean;
  next[1] = stdev;
  posdistr_->init(next);
...
// roll into parent....
Array<Tag*>* DiscrimValMixtureDistr::getMixtureDistrTags(const char* name) {
  char next[500];
...
Boolean DiscrimValMixtureDistr::update(Array<double>* probs) {
  Boolean result;
  if (nonparam_) {
...
Boolean DiscrimValMixtureDistr::finalupdate(Array<double>* probs) {
  if (nonparam_) {
...
Boolean DiscrimValMixtureDistr::reinitialize() {
...
Boolean DiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
Boolean DiscrimValMixtureDistr::initializeNegDistribution(NTTMixtureDistr* nttdistr) {
