...
Program       : MyrimatchDiscrimValMixtureDistr for PeptideProphet                                                       
...
static map<string, MyrimatchDiscrimFunctionFactory*>* pFactoryMap_ = NULL;

void MyrimatchDiscrimValMixtureDistr::registerDiscrimFunctionFactory(MyrimatchDiscrimFunctionFactory* factory) {
  if (pFactoryMap_ == NULL)
    pFactoryMap_ = new map<string, MyrimatchDiscrimFunctionFactory*>();
  string name = factory->getName();
  pFactoryMap_->insert(make_pair(name, factory));
}

// TODO: Figure out a better way to make sure these get linked in.
//extern void linkMyrimatchPscore();
extern void linkMyrimatchMVHscore();

MyrimatchDiscrimValMixtureDistr::MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof) {
  init(charge, engine, name, tag, maldi, qtof, False, True);													   
}																												   
																												   
MyrimatchDiscrimValMixtureDistr::MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
} 

MyrimatchDiscrimValMixtureDistr::MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, nonparam);
} 
void MyrimatchDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) {
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
}
void MyrimatchDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) {
  if (bFirstCall) {
	bFirstCall = false;
	// put the text of the "README" file where users can actually be aware of it - BSP
	std::cout << "WARNING!! The discriminant function for Myrimatch is not yet complete.  It is presented "
			"here to help facilitate trial and discussion.  Reliance on this "
  ...
  if (!nonparam) {
    cerr << "WARNING: Myrimatch only support semi-parametric PeptideProphet modelling, which relies on a DECOY search." << endl;
  }
  nonparam_ = True; //TODO: clean up code from parameteric stuff that doesn't apply to Myrimatch
  //  nonparam_ = nonparam;

  //  linkMyrimatchPscore();
  linkMyrimatchMVHscore();
  ...
  map<string, MyrimatchDiscrimFunctionFactory*>::iterator it = pFactoryMap_->find(algorithmName);

  if (it == pFactoryMap_->end()) {
    cerr << "Error: Myrimatch scoring algorithm '" << algorithmName << "' is not supported.";
    exit(1);
  }
...
Boolean MyrimatchDiscrimValMixtureDistr::update(Array<double>* probs) {
...
void MyrimatchDiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
    ((MyrimatchDiscrimFunction*)discrim_func_)->setMassSpecType(mass_spec_type);
}



double MyrimatchDiscrimValMixtureDistr::getPosProb(int index) { 
...
double MyrimatchDiscrimValMixtureDistr::getNegProb(int index) {
  if(all_negs_ || (*doublevals_)[index] < negmean_) {
    return 1.0;
  }
...
Boolean MyrimatchDiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
double MyrimatchDiscrimValMixtureDistr::getMinVal() {
  return (DiscrimValMixtureDistr::getMinVal());
  
}

double MyrimatchDiscrimValMixtureDistr::getMaxVal() {
