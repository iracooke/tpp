#ifndef Myrimatch_DISCRIMV_H
#define Myrimatch_DISCRIMV_H
...
class MyrimatchDiscrimFunctionFactory;

class MyrimatchDiscrimValMixtureDistr : public DiscrimValMixtureDistr {

 public:
  MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam);
  MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma);
  MyrimatchDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof);
 ...
 public:
  static void registerDiscrimFunctionFactory(MyrimatchDiscrimFunctionFactory* factory);
};

#include "Validation/MixtureDistribution/DiscrimFunctionFactory.h"
class MyrimatchDiscrimFunctionFactory : public DiscrimFunctionFactory
{
 public:
	 MyrimatchDiscrimFunctionFactory(const char* name) : DiscrimFunctionFactory(name)
  {
      MyrimatchDiscrimValMixtureDistr::registerDiscrimFunctionFactory(this);
  }

  virtual MyrimatchDiscrimFunction* createDiscrimFunction(int charge) = 0;
