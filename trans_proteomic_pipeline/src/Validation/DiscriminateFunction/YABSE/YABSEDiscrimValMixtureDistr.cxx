...
static map<string, YABSEDiscrimFunctionFactory*>* pFactoryMap_ = NULL;

void YABSEDiscrimValMixtureDistr::registerDiscrimFunctionFactory(YABSEDiscrimFunctionFactory* factory) {
  if (pFactoryMap_ == NULL)
    pFactoryMap_ = new map<string, YABSEDiscrimFunctionFactory*>();
  string name = factory->getName();
  pFactoryMap_->insert(make_pair(name, factory));
}

// TODO: Figure out a better way to make sure these get linked in.
extern void linkYABSELDAscore();
//extern void linkYABSEEscore();

YABSEDiscrimValMixtureDistr::YABSEDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof) {
  init(charge, engine, name, tag, maldi, qtof, False, True);											   
}																										   
																										   
YABSEDiscrimValMixtureDistr::YABSEDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
} 

YABSEDiscrimValMixtureDistr::YABSEDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, nonparam);
} 
void YABSEDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) {
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
}
void YABSEDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) {
  if (bFirstCall) {
	bFirstCall = false;
	// put the text of the "README" file where users can actually be aware of it - BSP
	std::cout << "WARNING!! The discriminant function for YABSE is not yet complete.  It is presented "
			"here to help facilitate trial and discussion.  Reliance on this "
  ...
  if (!nonparam) {
    cerr << "WARNING: YABSE only support semi-parametric PeptideProphet modelling, which relies on a DECOY search." << endl;
  }
  nonparam_ = True; //TODO: clean up code from parameteric stuff that doesn't apply to YABSE
  //  nonparam_ = nonparam;
  ...
  map<string, YABSEDiscrimFunctionFactory*>::iterator it = pFactoryMap_->find(algorithmName);

  if (it == pFactoryMap_->end()) {
    cerr << "Error: YABSE scoring algorithm '" << algorithmName << "' is not supported.";
    exit(1);
  }
  ...
  if (nonparam_) {
    negdistr_ = new NonParametricDistribution(0.2, True, False);
    posdistr_ = new NonParametricDistribution(0.2, False, False);
  }
  else {
    if(gammapos_) 
...
void YABSEDiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
    ((YABSEDiscrimFunction*)discrim_func_)->setMassSpecType(mass_spec_type);
}



double YABSEDiscrimValMixtureDistr::getPosProb(int index) { 
 
  if( (*doublevals_)[index] < negmean_) {
    return 0.0;
  }
...
double YABSEDiscrimValMixtureDistr::getNegProb(int index) {
...
Boolean YABSEDiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
double YABSEDiscrimValMixtureDistr::getMinVal() {
  return (DiscrimValMixtureDistr::getMinVal());
  
}

double YABSEDiscrimValMixtureDistr::getMaxVal() {
