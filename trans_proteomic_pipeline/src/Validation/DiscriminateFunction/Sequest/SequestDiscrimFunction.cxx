...
SequestDiscrimFunction::SequestDiscrimFunction(int charge, bool use_expect) : DiscriminantFunction(charge) {
...
Boolean SequestDiscrimFunction::isComputable(SearchResult* result) {
  return (result != NULL && ((SequestResult*)(result))->xcorr_ > 0.0);
}

double SequestDiscrimFunction::getDiscriminantScore(SearchResult* result) {
  if(strcasecmp(result->getName(), "Sequest") != 0) {
    cerr << "illegal type of Sequest result: " << result->getName() << endl;
    exit(1);
  }
  SequestResult* seqresult = (SequestResult*)(result);
  if (use_expect_) {
    //    if (seqresult->expect_ > 0) {
    //  return -5-log((double)seqresult->expect_);
    //}
    //else {
    //  cerr << "ERROR: Sequest Result for spectrum " << seqresult->spectrum_ 
    //	   << " does NOT have a positive expect score." << endl;
    //  return -100;
    //}
  }
  double tot = const_;
  ...
  tot += (double)seqresult->rank_ > 0 ? log_rank_wt_ * log((double)seqresult->rank_) : 0;

  
  if (! ((double)seqresult->rank_ > 0) ) {
    tot = (12 / 3.6) * seqresult->xcorr_ - 1.11;
  }
...
int SequestDiscrimFunction::getPepLen(char* pep) {
  if(pep == NULL)
    return 0;
  int start = 0;
...
double SequestDiscrimFunction::getXcorrP(double xcorr, int peplen) {
