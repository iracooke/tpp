...
static map<string, MSGFPLDiscrimFunctionFactory*>* pFactoryMap_ = NULL;

void MSGFPLDiscrimValMixtureDistr::registerDiscrimFunctionFactory(MSGFPLDiscrimFunctionFactory* factory) {
  if (pFactoryMap_ == NULL)
    pFactoryMap_ = new map<string, MSGFPLDiscrimFunctionFactory*>();
  string name = factory->getName();
  pFactoryMap_->insert(make_pair(name, factory));
}

// TODO: Figure out a better way to make sure these get linked in.
//extern void linkMSGFPLPscore();
extern void linkMSGFPL_MSGFScore();

MSGFPLDiscrimValMixtureDistr::MSGFPLDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof) {
  init(charge, engine, name, tag, maldi, qtof, False, True);													   
}																												   
																												   
MSGFPLDiscrimValMixtureDistr::MSGFPLDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
} 

MSGFPLDiscrimValMixtureDistr::MSGFPLDiscrimValMixtureDistr(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) { 
  init(charge, engine, name, tag, maldi, qtof, gamma, nonparam);
} 
void MSGFPLDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma) {
  init(charge, engine, name, tag, maldi, qtof, gamma, True);
}
void MSGFPLDiscrimValMixtureDistr::init(int charge, const char* engine, const char* name, const char* tag, Boolean maldi, Boolean qtof, Boolean gamma, Boolean nonparam) {
  if (bFirstCall) {
	bFirstCall = false;
	// put the text of the "README" file where users can actually be aware of it - BSP
	std::cout << "WARNING!! The discriminant function for MSGFPL is not yet complete.  It is presented "
			"here to help facilitate trial and discussion.  Reliance on this "
  ...
  if (!nonparam) {
    cerr << "WARNING: MSGFPL only support semi-parametric PeptideProphet modelling, which relies on a DECOY search." << endl;
  }
  nonparam_ = True; //TODO: clean up code from parameteric stuff that doesn't apply to MSGFPL
  //  nonparam_ = nonparam;

  //  linkMSGFPLPscore();
  linkMSGFPL_MSGFScore();
  ...
  map<string, MSGFPLDiscrimFunctionFactory*>::iterator it = pFactoryMap_->find(algorithmName);

  if (it == pFactoryMap_->end()) {
    cerr << "Error: MSGFPL scoring algorithm '" << algorithmName << "' is not supported.";
    exit(1);
  }
  ...
  if (nonparam_) {
    negdistr_ = new NonParametricDistribution(0.1, True, False);
    posdistr_ = new NonParametricDistribution(0.1, False, False);
  }
  else {
    if(gammapos_) 
...
Boolean MSGFPLDiscrimValMixtureDistr::update(Array<double>* probs) {
...
void MSGFPLDiscrimValMixtureDistr::setDiscrimFunction(const char* mass_spec_type) {
    ((MSGFPLDiscrimFunction*)discrim_func_)->setMassSpecType(mass_spec_type);
}



double MSGFPLDiscrimValMixtureDistr::getPosProb(int index) { 
...
double MSGFPLDiscrimValMixtureDistr::getNegProb(int index) {
  if(all_negs_ || (*doublevals_)[index] < negmean_) {
    return 1.0;
  }
...
Boolean MSGFPLDiscrimValMixtureDistr::initializeNegDistribution(Array<Boolean>* isdecoy) {
  if (nonparam_) {
    isdecoy_ = isdecoy;
...
double MSGFPLDiscrimValMixtureDistr::getMinVal() {
  return (DiscrimValMixtureDistr::getMinVal());
  
}

double MSGFPLDiscrimValMixtureDistr::getMaxVal() {
